...
A corrective-RAG chatbot that answers questions about AWS documentation (S3, EC2, DynamoDB) — and knows when *not* to trust its own retrieval.
...
4. **Generate** one answer from whatever's left (relevant local chunks + web results), with every source cited
...
app.py       Streamlit chat UI, wraps query_result() with history + caching display
```
...
- **Vector store**: Chroma, running as a local Python library (no server to manage)
...
Ingest the corpus (only needs to run once, or after changing `data/` or chunk settings):
...
Fixed corpus by design — an evaluation number is only meaningful if the corpus and test questions stay constant across every measurement.
...
- **Refusal** (3): genuinely unanswerable by *either* local corpus or the open web (fictional AWS features, private account data)
...
Grading was manual — reading each system answer against the expected answer and marking correct/incorrect myself, not automated exact-match or an LLM-as-judge. Every eval run's raw output is kept in `eval/results_*.md` for inspection.
...
- **Free-tier rate limits are real.** Gemini's free tier caps at ~15 requests/minute for the chat model; a single query makes ~8 calls (embedding + up to k relevance checks + optional query rewrite + generation), so heavy testing sessions routinely hit the limit and see multi-second internal retry delays.
- **Relevance grading is the single biggest latency cost** — historically 6 sequential LLM calls, now batched into 1 call per query (grading all retrieved chunks together), cutting that stage's cost roughly 6x.
- **Caching is exact-match only.** `cache.json` keys on the literal question string — rephrasing a question doesn't hit the cache even if it means the same thing.
